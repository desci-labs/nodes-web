...
export default function deserialize<T extends InputNodeTypes>(
  ...
  if (nodeChildren && Array.isArray(nodeChildren) && nodeChildren.length > 0) {
    ...
    /**
     * There is a need for a fix on the directory link void component.
     * Too many bugs unless I place an empty text node before it. This
     * was the best way to do it.
     */
    const firstChild = children[0] as LinkElement;
    if (firstChild.type === "link" && firstChild.link.startsWith("#/")) {
      children = [{ text: "" }, ...children];
    }

    /**
     * There is a need for a fix on the directory link void component.
     * Too many bugs unless I place an empty text node after it. This
     * was the best way to do it.
     */
    const lastChild = children[children.length - 1] as LinkElement;
    if (lastChild.type === "link" && lastChild.link.startsWith("#/")) {
  ...
  switch (node.type) {
    ...
    case "link":
      if (node.url?.startsWith("#/")) {
        return {
          ...
          fileName: node.children?.length && node.children[0].value,
          children: [{ text: "" }],
        } as LinkNode<T>;
      } else {
        return {
    ...
    case "math":
      return { type: types.math, children: [{ text: node.value }] } as Math<T>;
    ...
    case "inlineMath":
      return {
        [types.inlineMath as string]: true,
        text: node.value,
        ...persistLeafFormats(children as Array<MdastNode>),
      };
    case "text":
